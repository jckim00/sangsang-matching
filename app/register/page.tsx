...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error) {
      setErrors({ name: "저장 중 오류가 발생했습니다: " + error.message });
      return;
    }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {success && (
        <div className="mb-6 rounded-xl border-2 border-green-500 bg-green-50 px-6 py-4">
          <p className="text-xl font-bold text-green-700">등록이 완료되었습니다 ✓</p>
        </div>
      )}
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="flex flex-col gap-6" noValidate>
            ...
            <div className="flex flex-col gap-2">
              ...
              {errors.region && (
                <div className="rounded-lg border border-red-400 bg-red-50 px-4 py-2">
                  <p className="text-lg text-red-600">{errors.region}</p>
                </div>
              )}
              <Select
                value={form.region}
            ...
            <div className="flex flex-col gap-2">
              ...
              {errors.desired_job && (
                <div className="rounded-lg border border-red-400 bg-red-50 px-4 py-2">
                  <p className="text-lg text-red-600">{errors.desired_job}</p>
                </div>
              )}
              <Select
